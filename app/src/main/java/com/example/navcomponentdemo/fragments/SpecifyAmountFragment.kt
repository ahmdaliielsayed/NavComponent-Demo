...
import com.example.navcomponentdemo.Money
...
class SpecifyAmountFragment : Fragment(), View.OnClickListener {
    ...
    override fun onClick(v: View?) {
        when(v!!.id) {
            R.id.send_btn -> {
                if (!TextUtils.isEmpty(requireView().findViewById<TextInputEditText>(R.id.input_amount).text.toString())) {
                    val amount = Money(BigDecimal(requireView().findViewById<TextInputEditText>(R.id.input_amount).text.toString()))
                    val bundle = bundleOf(
